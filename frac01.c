...
MODULE_AUTHOR("M&M");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Simple congestion control algorithm with cubic growth in slow start and fixed percent growth in fast recovery and congestion avoidance.");
...
static u32 kcount = 0; // recovery counter
...
static void simple_cong_control(struct sock *sk, const struct rate_sample *rs)
{
	...
	if (rs->losses > 0) {
		...
		if (recovery_mode) {
			...
			tp->snd_cwnd += max(increment, 1U); // Ensure at least 1 is added
		} else {
            // congestion avoidance phase
			u32 increment = tp->snd_cwnd * 5 / 100;
...
{
	pr_info("CCA Fracionario: Initializing\n");
...
static void __exit simple_cong_exit(void)
{
	...
	pr_info("CCA Fracionario: Unloaded\n");
}
